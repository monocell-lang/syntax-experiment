...
Example 5: use reference to avoid both copy and move  
```
struct String {
   (Example 4)
...
Example 6: referencing a value 0_0?  
```
struct String {
   (Example 4)
...
```
```
   String s = init("World Wide Web");
   String::print(s);  //✅ reference, no mov, no .clone()
...
```
```
   String::print(init("Coco🥥 Choco"));  //wait what🤨🤔?
...
Example 7: referencing a value = ded #_#   
```
struct String {
   (Example 4)
...
Example 8: composite / struct field are references. surprised😵‍💫🤡?  
```
struct Person {
   String name,
   short age,
...
```
```
   String his_name = mov vilager.name;  //❌ cant mov reference🤡
...
Example 9: a field of a value is also a value  
```
struct Person {
   String name,
   short age,
